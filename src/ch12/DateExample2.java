...
public class DateExample2 {
	...
	public static void main(String[] args) {
		// Calender 객체 생성 - 추상 클래스로 getInstance()를 이용해서 생성
		...
		// cal2는 특정 시간대의 시간 정보로 생성된 Calender 객체가 됨.
...
//		System.out.println(Calendar.SATURDAY);
		...
		cal.set(Calendar.DATE, 1);	// 14 -> 1
		printDayOfSeries(cal);
		
		cal.set(Calendar.DAY_OF_WEEK, 5);	// 2 -> 5
		printDayOfSeries(cal);
		
		
		
		// 원하는 필드 더하기 .add(필드, 더할값)
		cal.add(Calendar.DATE, 20);		// 현재 저장된 값에서 20일 더하기
		printDayOfSeries(cal);
		
		cal.add(Calendar.DATE, -8);		// 현재 저장된 값에서 20일 더하기
		printDayOfSeries(cal);
	...
	static void printDayOfSeries(Calendar cal) {
		...
		// datOfWeekInMonth : 해당 요일이 그달의 몇번째 요일인지 체크
		int datOfWeekInMonth = cal.get(Calendar.DAY_OF_WEEK_IN_MONTH);
		...
		System.out.printf("dayOfyear : %d\n"
						...
						+ "dayOfWeek : %d \n"
						+ "dayOfWeekInMonth : %d \n",
						dayOfYear, dayOfMonth, dayOfWeek, datOfWeekInMonth);
