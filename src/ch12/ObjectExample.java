...
public class ObjectExample {
	...
	public static void main(String[] args) {
		...
		System.out.println(person.toString());	// 안에 무슨 값이 있는지 문자열로 보여줌
		
		// record 키워드를 사용한 경우 매개변수로 전달된 데이터의 값의 따라서 객체를 구분함
		// 같은 값을 전달하면 같은 객체로, 다른 값을 전달하면 다른 객체로 구분함.
		Person p1 = new Person("testuser1", 20);	// 넘겨주는 매개변수의 값이 같으면 같은 것이라고 간주함.
		Person p2 = new Person("testuser1", 20);	// 따라서 p1의 객체를 만들고 또 중복된 p2의 객체를 따로 만들지 않고 같은 주소를 반환함
		Person p3 = new Person("testuser2", 20);	// 그러나 값이 달라지면 다른 객체로 생성하기 때문에 다른 주소가 반환됨.
