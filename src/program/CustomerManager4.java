...
public class CustomerManager4 implements Serializable { // 직렬화

	private static final long serialVersionUID = 2946494067698087156L; // 값을 불러와서 사용하는 게 편함(직렬화 끝~!)
	 ...
	 * 메서드(writeCustomerData) -> 데이터 입력/수정시, 프로그램 종료시 데이터를 로드하는
	 * 메서드(readCustomerData) -> 프로그램 시작 전에 로드
	 * 
	 */
...
//	            cus.setName(scan.next());
	...
	// void로 처리할 수 있는 이유는 같은 클래스 내에 있는 메서드이므로 객체 접근이 가능하기 때문
	public static void writeCustomerData() {
		...
		} finally { // 객체 정리할 때 많이 사용
			try {
				if (oos != null)
					oos.close(); // 얘들도 예외가 발생할 수 있는 애들이므로 따로 한번 더 묶어줌. close는 객체를 정리하기 위함인데
				if (fos != null)
					fos.close(); // close는 객체를 정리하기 위함인데, 정리가 되어있으면 또 정리할 필요 없으니 조건문으로 한 번 확인하고 들어감
...
//		} finally {
//			try {
//				if (ois != null) ois.close();	// 얘들도 예외가 발생할 수 있는 애들이므로 따로 한번 더 묶어줌. close는 객체를 정리하기 위함인데 
