...
//            e.printStackTrace();
   ...
   static boolean addNewMember(String id) {		// 아이디 등록 처리하는 메서드
	   ...
	   id = id.trim().toLowerCase();	// 양 끝 공백 제거 및 소문자로 변환
	   ...
	   if(id.contains(" ")) {	// 입력한 값에 공백 여부 확인
		   ...
		   System.out.println("글자수 제한은 20글자 입니다.");
		   return false;
	   }
   ...
   // 2) 중복된 아이디인지 확인 chkIdDuple(id) 반환 타입 boolean = true(중복), false(중복X)
   static boolean chkIdDuple(String id) {
	   // memList에 동일한 id가 있는지 확인
...
//	   }
   ...
   // chkValidChar(id) 영어, 숫자, _만 가능. true(유효한 문자 사용), false(유효한 문자 사용 x)
   static boolean chkValidChar(String id) {
	   
	   String whiteList = "qwertyuiopasdfghjklzxcvbnm_1234657980";
